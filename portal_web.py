...
def main(page: ft.Page):
    ...
    vista_paso3 = ft.Column([
        ...
        ft.Container(content=texto_resumen_final, bgcolor=MUTED_COLOR, padding=15, border_radius=10, width=350), ft.Divider(color=CARD_COLOR),
        input_nombre, input_telefono, ft.Container(height=20),
        ft.Row([ft.TextButton("⬅️ Atrás", on_click=lambda _: cambiar_vista(vista_paso2), style=ft.ButtonStyle(color=ft.Colors.WHITE54)), btn_confirmar_final], alignment=ft.MainAxisAlignment.SPACE_BETWEEN, width=380)
    ], horizontal_alignment=ft.CrossAxisAlignment.CENTER, visible=False)

    vista_exito = ft.Column([
        ft.Container(height=50), ft.Icon(ft.Icons.CHECK_CIRCLE, color=ACCENT_COLOR, size=100),
        ft.Text("¡Cita Confirmada!", size=30, weight="bold", color=TEXT_WHITE), ft.Text("Tu espacio ha sido reservado.", color=ft.Colors.WHITE70),
        ft.Container(height=30), ft.ElevatedButton("Volver al Menú", bgcolor=CARD_COLOR, color=TEXT_WHITE, on_click=reiniciar_proceso)
    ], horizontal_alignment=ft.CrossAxisAlignment.CENTER, visible=False)
                ...
                for c in mis_citas:
                    def cancelar_esta(e, cid=c.get('id')):
                        try:
                            ...
                            page.show_dialog(ft.SnackBar(ft.Text(f"Error: {ex}"), bgcolor=ft.Colors.RED, open=True))
                    lista_citas_cancelar.controls.append(ft.Container(content=ft.Row([ft.Text(f"📅 {c.get('fecha')} - {c.get('hora')}\n💆‍♀️ {c.get('servicio')}", size=13, color=TEXT_WHITE, expand=True), ft.ElevatedButton("Cancelar", icon=ft.Icons.DELETE, bgcolor=ft.Colors.RED_700, color=TEXT_WHITE, on_click=lambda e, cid=c.get('id'): cancelar_esta(e, cid))], alignment=ft.MainAxisAlignment.SPACE_BETWEEN), bgcolor=CARD_COLOR, padding=15, border_radius=15, border=ft.border.all(1, ft.Colors.RED_900)))
        except Exception as ex:
    ...
    vista_cancelar = ft.Column([
        ft.Text("CANCELAR CITA", size=20, weight="bold"), ft.Text("Ingresa el número con el que agendaste.", color=ft.Colors.WHITE54, text_align=ft.TextAlign.CENTER), ft.Container(height=10),
        input_cancelar_tel, btn_buscar_citas, ft.Divider(color=CARD_COLOR), lista_citas_cancelar, ft.Container(height=20),
    ...
    vista_lealtad = ft.Column([
        ft.Text("PROGRAMA DE LEALTAD", size=14, weight="bold", color=ACCENT_COLOR), ft.Text("6to Masaje GRATIS", size=20, weight="bold", color=TEXT_WHITE), ft.Container(height=10),
        input_wa_lealtad, btn_verificar_lealtad, ft.Container(height=10), grid_sellos, mensaje_lealtad, ft.Container(height=30),
        ft.TextButton("⬅️ Volver al Menú", on_click=reiniciar_proceso, style=ft.ButtonStyle(color=ft.Colors.WHITE54))
    ], horizontal_alignment=ft.CrossAxisAlignment.CENTER, visible=False)
