...
def main(page: ft.Page):
    ...
    def check_admin_pwd(e):
        if admin_pwd_field.value == "3157525":
            admin_dlg.open = False; page.update(); page.go("/admin")
        else:
            admin_pwd_field.error_text = "Contraseña incorrecta"; page.update()

    admin_dlg = ft.AlertDialog(title=ft.Text("Acceso Administrativo", color=COLOR_RESPIRO), content=admin_pwd_field, actions=[ft.TextButton("Cancelar", on_click=lambda _: setattr(admin_dlg, 'open', False) or page.update()), ft.ElevatedButton("Entrar", bgcolor=COLOR_RESPIRO, color=COLOR_TEXTO_BLANCO, on_click=check_admin_pwd)], actions_alignment=ft.MainAxisAlignment.END)
    ...
    def RespiroPricingCard(title, price, savings_text, features_list, package_id):
        ...
        return ft.Container(
            content=ft.Column([
                ...
                ft.Text(savings_text, size=13, color=COLOR_RESPIRO_DARK), ft.Divider(height=30, color="#E5E5EA"), 
                ft.Column(features_ui, spacing=10), ft.Container(height=10), 
    ...
    def route_change(e):
        ...
        if page.route == "/login":
            page.views.append(ft.View("/login", bgcolor=COLOR_RESPIRO, padding=0, controls=[
                ft.Container(expand=True, padding=ft.padding.only(left=30, right=30, top=40, bottom=40), content=ft.Column([
        ...
        # 1.5 FORMULARIO
            ...
            def do_login(e):
                if nombre_field.value and celular_field.value:
                    ...
                    page.session.set("user_phone", celular_field.value); page.session.set("user_name", nombre_final); page.session.set("has_active_package", tiene_paquete_activo)
        ...
        # 3. PAGOS Y VERIFICACIÓN
            ...
            def pagar_bbva(e):
                ...
                if link:
                    ...
                    btn_bbva.bgcolor = ft.colors.GREEN_600; btn_bbva.url = link; btn_bbva.on_click = lambda _: page.go("/pago/verificando"); page.update()
        ...
        # =========================================================================
            ...
            def recargar_pantalla():
                ...
                for i in range(14): 
                    ...
                    dias_ui.controls.append(ft.Container(content=ft.Column(col_dia, alignment=ft.MainAxisAlignment.CENTER, horizontal_alignment=ft.CrossAxisAlignment.CENTER, spacing=0), bgcolor=bg_color, width=65, height=85, border_radius=15, border=ft.border.all(1, "#E5E5EA") if not es_dia_seleccionado else None, on_click=lambda e, f=fecha_iter: al_seleccionar_dia(f)))

                horarios_ui.controls.clear() 
                fecha_str = str(vista_estado["fecha_activa"])
                
                if AppDB.es_dia_bloqueado(fecha_str):
                    horarios_ui.controls.append(ft.Container(content=ft.Column([ft.Icon(ft.icons.NIGHTLIGHT_ROUND, size=50, color=COLOR_RESPIRO), ft.Text("Estudio Cerrado", size=20, weight=ft.FontWeight.BOLD, color=COLOR_TEXTO_OSCURO), ft.Text("Tómate un descanso. Nos vemos pronto.", text_align=ft.TextAlign.CENTER, color=COLOR_RESPIRO_DARK)], alignment=ft.MainAxisAlignment.CENTER, horizontal_alignment=ft.CrossAxisAlignment.CENTER), bgcolor=COLOR_TEXTO_BLANCO, padding=40, border_radius=15, width=float('inf')))
                else:
                    ...
                    for h in clases_del_dia:
                        ...
                        horarios_ui.controls.append(ft.Container(content=ft.Row([ft.Column([ft.Text(h["hora"], size=18, weight=ft.FontWeight.BOLD, color=COLOR_TEXTO_OSCURO), ft.Text(f"Prof: {h['instructor']}", size=13, color=COLOR_RESPIRO_DARK), ft.Text(f"Lugares: {h['cupo']}", size=12, color=COLOR_RESPIRO if not is_full else ft.colors.RED_400)], spacing=2), ft.Container(expand=True), ft.Container(content=ft.Text(btn_text, weight=ft.FontWeight.BOLD, color=text_btn_color, size=13), bgcolor=btn_color, padding=ft.padding.symmetric(horizontal=20, vertical=10), border_radius=20, on_click=accion_btn)]), bgcolor=COLOR_TEXTO_BLANCO, padding=20, border_radius=15, shadow=ft.BoxShadow(blur_radius=10, color="#0A000000", offset=ft.Offset(0, 4))))
                page.update()

            def al_seleccionar_dia(nueva_fecha): vista_estado["fecha_activa"] = nueva_fecha; recargar_pantalla()
            def al_seleccionar_servicio(nuevo_servicio): vista_estado["servicio_activo"] = nuevo_servicio; recargar_pantalla()
            ...
            avatar_perfil = ft.Container(content=ft.Text(inicial, size=22, weight=ft.FontWeight.BOLD, color=COLOR_TEXTO_BLANCO), alignment=ft.alignment.center, width=45, height=45, bgcolor=COLOR_RESPIRO, shape=ft.BoxShape.CIRCLE, shadow=ft.BoxShadow(blur_radius=5, color="#33000000", offset=ft.Offset(0, 2)), on_click=lambda _: page.go("/perfil"))

            page.views.append(ft.View("/servicios", bgcolor=COLOR_BG_CLARO, padding=20, scroll=ft.ScrollMode.AUTO, controls=[
                ft.Container(height=10), ft.Row([ft.Column([ft.Text(f"Hola, {primer_nombre} 👋", size=26, weight=ft.FontWeight.BOLD, color=COLOR_TEXTO_OSCURO), ft.Text("Reserva tu próxima clase", size=15, color=COLOR_RESPIRO_DARK)], spacing=0), ft.Container(expand=True), avatar_perfil]),
                ft.Container(height=25), servicios_ui, ft.Container(height=25), dias_ui, ft.Container(height=25), ft.Text("Horarios Disponibles", size=16, weight=ft.FontWeight.BOLD, color=COLOR_TEXTO_OSCURO), ft.Container(height=10), horarios_ui, ft.Container(height=40) 
            ]))
            
        # =========================================================================
        # 4.5 HISTORIAL Y PERFIL (RECARGA FORZADA)
        # =========================================================================
            ...
            # --- FUNCIÓN CLAVE PARA EVITAR PANTALLA BLANCA ---
            def forzar_recarga_perfil():
                page.route = "/perfil"
                route_change(None) # Esto limpia la vista y la dibuja desde cero sin tocar el router del navegador
            ...
            def refrescar_creditos_click(e):
                ...
                page.overlay.append(snack); snack.open = True
                forzar_recarga_perfil() # <--- Llamada a la recarga limpia

            boton_refrescar = ft.Container(content=ft.Row([ft.Icon(ft.icons.REFRESH, color=COLOR_RESPIRO, size=20), ft.Text("¿Pagaste y no ves tus clases? Toca aquí", color=COLOR_RESPIRO, size=13, weight=ft.FontWeight.BOLD)], alignment="center"), padding=12, border=ft.border.all(1, COLOR_RESPIRO), border_radius=10, on_click=refrescar_creditos_click, margin=ft.margin.only(bottom=15))

            reserva_a_cancelar = [None]; aplica_penalizacion = [False]
            
            def ejecutar_cancelacion_final(e):
                rid = reserva_a_cancelar[0]; penalizar = aplica_penalizacion[0]
                dlg_cancelar.open = False
                contenedor_reservas.content = ft.Container(content=ft.Column([ft.ProgressRing(color=COLOR_RESPIRO), ft.Text("Procesando cancelación...", color=COLOR_RESPIRO_DARK)], horizontal_alignment=ft.CrossAxisAlignment.CENTER), alignment=ft.alignment.center, height=200); page.update()
                ...
                time.sleep(0.5) # Le damos medio segundo a Supabase para reaccionar
            ...
            dlg_cancelar = ft.AlertDialog(title=ft.Text("Cancelar Sesión", weight=ft.FontWeight.BOLD), content=ft.Text(""), actions=[ft.TextButton("Volver", on_click=lambda _: setattr(dlg_cancelar, 'open', False) or page.update()), ft.ElevatedButton("Sí, Cancelar", bgcolor=ft.colors.RED_500, color="white", on_click=ejecutar_cancelacion_final)])
            ...
            def preparar_cancelacion(res):
                ...
                except Exception as e: print("Error calculando tiempo:", e)
                
                aplica_penalizacion[0] = es_penalizable
                if es_penalizable: dlg_cancelar.content = ft.Text("Faltan menos de 12 horas para tu clase. Si cancelas ahora perderás esta clase según nuestra política.\n\n¿Deseas cancelarla de todos modos?", color=ft.colors.RED_600)
                else: dlg_cancelar.content = ft.Text("¿Estás seguro de que deseas cancelar tu clase?\n\nTu crédito será devuelto automáticamente a tu perfil.")
            ...
            page.views.append(ft.View("/perfil", bgcolor=COLOR_BG_CLARO, padding=20, scroll="auto", controls=[
                ...
                contenedor_reservas, ft.Container(height=30), ft.TextButton("Cerrar Sesión", icon=ft.icons.LOGOUT, style=ft.ButtonStyle(color=ft.colors.RED_400), on_click=lambda _: page.go("/login"))
            ]))
        ...
        # =========================================================================
            ...
            def set_modo_30(e):
                modo_agenda[0] = "30"; btn_modo_30.bgcolor, btn_modo_30.color = COLOR_RESPIRO, "white"; btn_modo_dia.bgcolor, btn_modo_dia.color = "transparent", COLOR_RESPIRO; recargar_listas()
                
            def set_modo_dia(e):
                modo_agenda[0] = "dia"; btn_modo_dia.bgcolor, btn_modo_dia.color = COLOR_RESPIRO, "white"; btn_modo_30.bgcolor, btn_modo_30.color = "transparent", COLOR_RESPIRO; recargar_listas()
            ...
            def ver_detalles_clase(cid):
                ...
                else: alumnos = [{"full_name": "Agrega la función a database.py", "active_package": "pagado"}]

                dlg_detalles.content.controls.clear()
                if not alumnos:
                    dlg_detalles.content.controls.append(ft.Text("No hay alumnos inscritos aún.", color=COLOR_RESPIRO_DARK))
                else:
                    dlg_detalles.content.controls.append(ft.Text(f"Total inscritos: {len(alumnos)}", weight="bold"))
                    for al in alumnos:
                        ...
                        dlg_detalles.content.controls.append(ft.ListTile(leading=ft.Icon(ft.icons.PERSON, color=color_icono), title=ft.Text(nombre_al, weight="bold"), subtitle=ft.Text(f"Estado: {estado_al.capitalize()}")))
                page.update()
                    ...
                    if not c.get('is_blocked'):
                        ...
                        lista_agenda.controls.append(ft.Container(
                            content=ft.Row([
                                ...
                                ft.Row([
                                    ft.IconButton(ft.icons.INFO_OUTLINE, icon_color=COLOR_RESPIRO, on_click=lambda e, cid=c['id']: ver_detalles_clase(cid)),
            ...
            def accion_publicar_clase(e):
                ...
                if AppDB.es_dia_bloqueado(f_str):
                    snack = ft.SnackBar(ft.Text("¡Error! Este día está marcado como Cerrado."), bgcolor=ft.colors.RED_500); page.overlay.append(snack); snack.open = True
                elif AppDB.verificar_disponibilidad(f_str, drop_hora.value):
                    snack = ft.SnackBar(ft.Text("¡Horario ocupado! Revisa la agenda."), bgcolor=ft.colors.RED_500); page.overlay.append(snack); snack.open = True
                    ...
                    snack = ft.SnackBar(ft.Text("Clase creada."), bgcolor=ft.colors.GREEN_600); page.overlay.append(snack); snack.open = True
            ...
            def accion_bloquear_dia(e):
                ...
                if f_str:
                    if not AppDB.es_dia_bloqueado(f_str):
                        ...
                        snack = ft.SnackBar(ft.Text(f"El día {f_str} ha sido bloqueado."), bgcolor=ft.colors.GREEN_600); txt_fecha_bloqueo.value = ""
                    else: snack = ft.SnackBar(ft.Text("Ese día ya está bloqueado."), bgcolor=ft.colors.ORANGE_500)
                else: snack = ft.SnackBar(ft.Text("Por favor, selecciona una fecha en el calendario."), bgcolor=ft.colors.RED_500)
                page.overlay.append(snack); snack.open = True; page.update()

            recargar_listas()

            tab_crear = ft.Tab(text="Crear", icon=ft.icons.ADD_BOX, content=ft.Container(padding=20, content=ft.Column([ft.Text("Agendar Nueva Clase", size=18, weight=ft.FontWeight.BOLD, color=COLOR_RESPIRO), ft.Container(height=10), drop_serv, drop_hora, ft.Row([txt_inst, txt_cupo]), ft.Container(height=20), ft.ElevatedButton("Publicar", bgcolor=COLOR_RESPIRO, color="white", width=float('inf'), height=50, on_click=accion_publicar_clase)])))
            tab_agenda = ft.Tab(text="Agenda", icon=ft.icons.FORMAT_LIST_BULLETED, content=ft.Container(padding=20, content=ft.Column([ft.Row([btn_modo_30, btn_modo_dia], spacing=10), ft.Container(height=10), lista_agenda])))
            tab_bloqueos = ft.Tab(text="Cierres", icon=ft.icons.BLOCK, content=ft.Container(padding=20, content=ft.Column([ft.Text("Días Inhábiles", size=18, weight=ft.FontWeight.BOLD, color=ft.colors.RED_400), ft.Text("Elige cualquier fecha futura para cerrar el estudio.", size=12, color=COLOR_RESPIRO_DARK), ft.Container(height=10), ft.Row([txt_fecha_bloqueo, ft.IconButton(ft.icons.CALENDAR_MONTH, icon_color=ft.colors.RED_400, icon_size=35, on_click=lambda _: dp_bloqueo.pick_date())]), ft.Container(height=10), ft.ElevatedButton("Bloquear Fecha", bgcolor=ft.colors.RED_400, color="white", width=float('inf'), height=50, on_click=accion_bloquear_dia), ft.Container(height=20), ft.Text("Próximos cierres:", weight=ft.FontWeight.BOLD), lista_bloqueos])))
            ...
            page.views.append(ft.View("/admin", bgcolor=COLOR_BG_CLARO, padding=0, controls=[
                ft.Container(bgcolor="white", padding=ft.padding.only(left=10, right=20, top=20, bottom=10), shadow=ft.BoxShadow(blur_radius=5, color="#1A000000"), content=ft.Row([
                    ...
                    ft.Container(expand=True), txt_fecha_top, ft.IconButton(ft.icons.CALENDAR_MONTH, icon_color=COLOR_RESPIRO, on_click=lambda _: dp_admin.pick_date())
                ])),
